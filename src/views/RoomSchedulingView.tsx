...
export const RoomSchedulingView: React.FC = () => {
    ...
    const [selectedSlot, setSelectedSlot] = useState<(RoomBooking | Omit<RoomBooking, 'id' | 'createdAt' | 'createdBy' | 'updatedAt' | 'updatedBy'>) | null>(null);
        ...
        <div id="print-schedule-content" style={{ display: 'none' }}>
            ...
            <table className="print-table">
                <thead>
                    <tr>
                        <th>שעה</th>
                        {rooms.map((room: any) => <th key={room.id}>{room.name}</th>)}
                    </tr>
                </thead>
                <tbody>
                    {timeSlots.map(time => (
                            ...
                            {rooms.map((room: any) => {
                                ...
                                if (bookingForSlot && time === bookingForSlot.startTime) {
                                    if (bookingForSlot.isBlocked) {
                                        ...
                                        content = therapist ? (therapist as Therapist).name : 'משובץ';
                                    }
                                }
        ...
        <>
            <Box>
                <Grid container justifyContent="space-between" alignItems="center" sx={{ mb: 3 }} className="no-print">
                    <Grid>
                        <Typography variant="h4" sx={{ fontWeight: '700' }}>שיבוץ חדרים</Typography>
                    </Grid>
                                    ...
                                    {rooms.map((room: any) => {
                                        ...
                                        if (bookingForSlot) {
                                            ...
                                            if (bookingForSlot.isBlocked) {
                                                ...
                                                tooltipText = `משובץ: ${(therapist as Therapist).name}`;
                                                if (isStartOfBooking) {
                                                    cellContent = <Chip label={(therapist as Therapist).name} size="small" color="primary" sx={{width: '100%'}} />;
                                                }
                                            }
                                        }
